...
lab_dx <- c(0, 0, -8, 6, 15)  # nudge the crowded short-phase labels apart
for (i in seq_along(ph)) {
  p <- ph[[i]]
  x0 <- as.numeric(p[1]); x1 <- as.numeric(p[2])
  rect(x0, 3.5, x1, 6.5, col = p[4], border = base_blue)
  text((x0 + x1) / 2 + lab_dx[i], 7.4, p[3], cex = 0.8, font = 2, col = base_blue, srt = 0)
}
# axis ticks
...
points(100, 50, pch = 19, col = accent, cex = 1.0); text(100, 50, "Guangzhou", pos = 3, cex = 0.7, col = "#222222")
