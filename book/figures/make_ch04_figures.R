...
# faint westward seed arrow (points toward the Atlantic end, now on the left)
arrows(0.70, 0.50, 0.60, 0.50, length = 0.07, lwd = 1.5, col = grey, lty = 2)
text(0.60, 0.30, "centre firmly Asian (Song China leading) - but the first faint", col = "#333333", font = 3, cex = 0.86)
text(0.60, 0.20, "north-western counter-current appears: a seed, not a shift", col = accent, font = 3, cex = 0.86)
...
points(20, 20, pch = 19, col = accent, cex = 0.8); text(20, 20, "Aden\n(the junction)", col = accent, cex = 0.6, font = 2, pos = 4)
...
# re-wrapped and shifted east so the block clears the dashed Grand Canal line
text(95, 63, "the world's first urban-commercial\nrevolution: iron ~125k t/yr;\npaper money; ~6bn coins/yr", cex = 0.58, col = grey, font = 3)
...
# label set inland (pos 2): to the seaward side the coastline cut straight through the word
points(98, 46, pch = 19, col = accent, cex = 1.2); text(98, 46, "Quanzhou (Zaiton)", pos = 2, cex = 0.68)
segments(72, 76, 80, 50, col = base_blue, lwd = 2, lty = 2); text(87, 54, "Grand Canal\n(~1,800 km)", cex = 0.56, col = base_blue, srt = -68)
...
points(40, 50, pch = 19, col = accent, cex = 0.9); text(36, 46, "Malabar\n(pepper)", pos = 2, cex = 0.62)
# west to Aden
...
for (i in seq_along(ph)) { p <- ph[[i]]; x0 <- as.numeric(p[1]); x1 <- as.numeric(p[2]); y <- ypos[i]
  ...
  if (x1 - x0 < 40) {  # band too narrow for an inside label: set it outside on a leader
    segments(x1, y, x1 + 5, y, col = base_blue)
    text(x1 + 5, y, p[3], cex = 0.64, font = 2, col = base_blue, pos = 4)
  } else {
...
# one label per dot, on two staggered rows so neighbours never share a baseline
text(1025, 9.6, "1025 Chola raid", cex = 0.6, col = accent)
text(1206, 8.7, "1206 Chinggis", cex = 0.6, col = accent)
text(1258, 9.6, "1258 Baghdad sacked", cex = 0.6, col = accent)
text(1346, 8.7, "1346 Black Death", cex = 0.6, col = accent)
text(1368, 9.6, "1368 Ming founded", cex = 0.6, col = accent)
...
yt <- c(100, 150, 200, 250); segments(1300, yt, 1310, yt, col = grey); text(1298, yt, yt, cex = 0.7, col = grey, pos = 2, xpd = NA)
