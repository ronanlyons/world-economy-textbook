...
arrows(0.70, 0.50, 0.58, 0.50, length = 0.07, lwd = 1.8, col = grey, lty = 2)
text(0.58, 0.28, "still firmly Asian: China the great silver sink that", col = "#333333", font = 3, cex = 0.86)
text(0.58, 0.18, "organised the first global circuit; the Atlantic seed grows", col = accent, font = 3, cex = 0.86)
...
arrows(15.8, 17.2, 64, 22, length = 0.09, lwd = 2.2, col = accent)         # Potosi -> Manila (Pacific)
...
arrows(13.6, 15.1, 20.2, 11.6, length = 0.07, lwd = 1.8, col = grey, lty = 2)
...
points(96, 48, pch = 19, col = accent, cex = 1.1); text(96, 48, "Canton/Macau", pos = 2, cex = 0.64)
points(86, 40, pch = 19, col = accent, cex = 0.9); text(86, 36.4, "Manila trade", cex = 0.6)
# silver in
arrows(122, 38, 101, 44.2, length = 0.09, lwd = 2.4, col = accent); text(126, 32, "silver in\n(Manila, Macau)", cex = 0.6, col = accent, pos = 2)
...
# ---------------------------------------------------------------------------
# 5. Phase timeline
png("phase-timeline-05.png", width = 1700, height = 700, res = 150)
par(mar = c(3, 1, 3, 1))
plot(NA, xlim = c(1400, 1610), ylim = c(0, 12), axes = FALSE, xlab = "", ylab = "")
ph <- list(
  c(1405, 1433, "1 apogee abandoned", "#cdddee"),
  c(1498, 1515, "2 European\nentry",  "#e8e0d0"),
...
# one label per dot
text(1498, 11.3, "1498 da Gama", cex = 0.62, col = accent)
text(1545, 11.3, "1545 Potosi silver", cex = 0.62, col = accent)
text(1571, 11.3, "1571 Manila galleon", cex = 0.62, col = accent)
...
text(mean(b), 17.3, "silver dear (cheap gold) in Europe, cheap (dear gold) in Asia", cex = 0.74, col = accent, font = 2)
text(mean(b), 16.1, "-> silver bought the most in Asia, so arbitrage pulled it east", cex = 0.74, col = accent, font = 2)
...
# Raw NOAA WDS-Paleo files staged in ../data/ (header/data-start lines confirmed).
{
  ...
  par(mar = c(5.4, 4.8, 3.0, 1.2), xaxs = "i")
  ...
  mtext(sprintf("Native-annual tree-ring reconstructions to 1600 CE. Coldest 30-yr window: Europe %d, N. America %d, S. America %d. Sources: Buntgen 2006; Wiles 2014; Neukom 2011 (NOAA WDS-Paleo).",
        round(te["year"]), round(tn["year"]), round(ts["year"])), side = 1, line = 4.1, cex = 0.54, col = grey)
  dev.off()
}
