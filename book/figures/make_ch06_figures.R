...
arrows(1 - 0.32, 0.50, 1 - 0.44, 0.50, length = 0.07, lwd = 1.8, col = grey, lty = 2)
text(0.50, 0.28, "still firmly Asian in PRODUCTION: India ~24.5% of world manufacturing (1750),", col = "#333333", font = 3, cex = 0.82)
text(0.50, 0.17, "silver still flows east; but Europe now holds the armed company, the navy, Atlantic capital", col = accent, font = 3, cex = 0.82)
...
arrows(86, 23, 74, 28.2, length = 0.08, lwd = 1.8, col = base_blue, lty = 3) # spice -> India/west
...
arrows(30, 28.4, 30, 18.8, length = 0.08, lwd = 2, col = grey)        # Europe goods/cloth -> W. Africa (straight down between the two boxes)
...
points(34, 70, pch = 19, col = accent, cex = 1.0); text(32, 70, "Gujarat\n(cotton)", pos = 2, cex = 0.58)
...
points(95, 47, pch = 19, col = accent, cex = 1.2); text(94, 48, "Canton\n(tea, silk,\nporcelain)", pos = 2, cex = 0.6)
...
par(mar = c(3, 1, 3, 1))
plot(NA, xlim = c(1595, 1782), ylim = c(0, 10), axes = FALSE, xlab = "", ylab = "")
ph <- list(
  ...
  c(1757, 1765, "6 Plassey (the turning point)", "#bcd2e8"))
ypos <- c(8.0, 8.0, 6.0, 4.0, 2.2, 2.2)
# label placement: 1 sits in the part of its band box 2 does not cover; 6's band is too
# narrow for its text, so the label is set beside the box on two lines
labx  <- c(1615, NA, NA, NA, NA, 1766.5)
labadj <- c(0.5, 0.5, 0.5, 0.5, 0.5, 0)
labtx <- c(ph[[1]][3], ph[[2]][3], ph[[3]][3], ph[[4]][3], ph[[5]][3], "6 Plassey\n(the turning point)")
for (i in seq_along(ph)) { p <- ph[[i]]; x0 <- as.numeric(p[1]); x1 <- as.numeric(p[2]); y <- ypos[i]
  ...
  xt <- if (is.na(labx[i])) (x0 + x1) / 2 else labx[i]
  text(xt, y, labtx[i], cex = 0.6, font = 2, col = base_blue, adj = c(labadj[i], 0.5)) }
...
# event labels sit on the dot row itself, clear of the band boxes above (bottom edge 1.35)
# and of the tick marks below (top 0.95); each is set to the side of its dot that has space
evside <- c(4, 4, 2, 2)
for (i in seq_along(ev)) { e <- ev[[i]]; ex <- as.numeric(e[1])
  points(ex, 1.05, pch = 19, cex = 0.6, col = accent)
  text(ex, 1.05, e[2], pos = evside[i], offset = 0.3, cex = 0.58, font = 3, col = accent) }
...
text(7.4, 6.7, "WEST AFRICA", font = 2, col = base_blue, cex = 0.78)
text(7.4, 5.95, "commodity-money flood +\ncaptive/raw-gold export\n-> divergence + depletion", font = 3, col = grey, cex = 0.6)
...
# silver imports as bars (right axis) first, so the ratio line sits on top of them
...
text(1638, 645, "600 t", col = base_blue, cex = 0.74, font = 2)
text(1643, 268, "<250 t", col = base_blue, cex = 0.74, font = 2)
...
# per-point label offsets keep each label clear of the rising line
text(yr + c(0, -0.3, 0, 0), ratio + c(320, 620, 480, 320),
     paste0("1:", format(ratio, big.mark = ",")), col = accent, cex = 0.72, font = 2)
