...
# map convention: west on the left, east on the right (every locator x is mirrored, x' = 1 - x)
# 1900: Asia 25.6% of world income, North Atlantic 53.2%, so 0.33 on the two-pole
# scale. It was 0.12, which drew a world where Asia had almost nothing.
x <- 1 - (0.05 + 0.67 * 0.90)
...
# commodities out
arrows(64, 60, 96, 66, length = 0.09, lwd = 2.2, col = base_blue); text(99, 70, "cotton, jute,\nwheat, tea ->", cex = 0.52, col = base_blue, pos = 2)
# manufactures in
...
points(98, 50, pch = 19, col = accent, cex = 1.1); text(96, 52, "treaty ports\n(tariff fixed 5%, 1842;\nforeign-run Customs)", pos = 2, cex = 0.54)
arrows(118, 44, 101, 49, length = 0.09, lwd = 2.2, col = grey); text(126, 39, "foreign\nmanufactures", cex = 0.52, col = grey, pos = 2)
arrows(96, 46, 80, 30, length = 0.09, lwd = 2, col = base_blue); text(77, 27, "tea, silk ->", cex = 0.54, col = base_blue, pos = 2)
...
ypos <- c(9.1, 9.1, 7.1, 5.1, 3.2)   # phases lifted to clear a labelled event row below
...
# event markers with their labels restored: staggered on two rows so neighbours never share a baseline
evy <- c(2.30, 1.72, 2.30, 1.72)
for (i in seq_along(ev)) { e <- ev[[i]]; x <- as.numeric(e[1])
  points(x, 1.25, pch = 19, cex = 0.6, col = accent)
  segments(x, 1.32, x, evy[i] - 0.27, col = accent, lwd = 0.8)
  text(x, evy[i], e[2], cex = 0.58, font = 2, col = accent) }
...
text(b[2], 50, "same integration,\nopposite terms", col = base_blue, font = 3, cex = 0.72)
...
arrows(69, 10, 34, 10, length = 0.09, lwd = 2, col = base_blue, lty = 2)
text(54, 4, "India's commodity surplus is earned in third countries' currencies", cex = 0.58, col = base_blue, font = 3)
