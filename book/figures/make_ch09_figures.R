...
png("cog-locator-09.png", width = 1400, height = 560, res = 150)
par(mar = c(2.5, 1, 2.5, 1))
plot(NA, xlim = c(0, 1), ylim = c(0, 1), axes = FALSE, xlab = "", ylab = "")
rect(0.05, 0.52, 0.95, 0.70, col = land, border = base_blue, lwd = 2)
text(0.05, 0.86, "NORTH ATLANTIC", col = base_blue, font = 2, cex = 1.25, adj = 0)
text(0.95, 0.86, "ASIA", col = base_blue, font = 2, cex = 1.25, adj = 1)
segments(seq(0.05, 0.95, length.out = 5), 0.50, seq(0.05, 0.95, length.out = 5), 0.72, col = base_blue, lwd = 1)
# On this bar the marker is Asia's income over Asia's plus the North Atlantic's: 1900 = 0.33,
# 1940 = 0.29, 1950 = 0.23. So the period drifts WEST, and ends west of chapter 8 -- but only
# to about a quarter, not to the pole. The old figure put it at 0.06, which drew a world with
# no Asia in it. London and New York are both inside the North Atlantic and now sit on their
# own sub-scale below, rather than borrowing this one.
x0 <- 0.05 + 0.33 * 0.90; x1 <- 0.05 + 0.26 * 0.90
points(x0, 0.61, pch = 19, cex = 2.4, col = grey); text(x0, 0.77, "1914", cex = 0.7, col = grey)
arrows(x0 - 0.016, 0.61, x1 + 0.022, 0.61, length = 0.06, lwd = 2, col = accent)
points(x1, 0.61, pch = 19, cex = 3.0, col = accent)
points(x1, 0.61, pch = 21, cex = 3.0, col = base_blue, lwd = 2)
text(x1, 0.77, "1945", cex = 0.72, col = accent, font = 2)
# the within-West shift, on its own scale so the two are not confused
segments(0.05, 0.40, 0.34, 0.40, col = grey, lwd = 1.4)
segments(c(0.05, 0.34), 0.375, c(0.05, 0.34), 0.425, col = grey, lwd = 1.4)
text(0.355, 0.40, "inside the North Atlantic", col = grey, font = 3, cex = 0.66, adj = 0)
points(0.235, 0.40, pch = 19, cex = 1.9, col = grey); text(0.235, 0.30, "London", cex = 0.66, col = grey)
arrows(0.215, 0.40, 0.135, 0.40, length = 0.05, lwd = 1.6, col = accent)
points(0.115, 0.40, pch = 19, cex = 2.1, col = accent); text(0.115, 0.30, "New York", cex = 0.68, col = accent, font = 2)
text(0.5, 0.17, "the European-centred order cracks, and the decisive shift is within the West (London -> New York)", col = "#333333", font = 3, cex = 0.76)
text(0.5, 0.07, "Asia's weight slips further, to about a quarter of the two-pole total by 1945 -- its lowest of the whole module", col = green, font = 3, cex = 0.76)
...
par(mar = c(3, 1, 3, 1))
plot(NA, xlim = c(1912, 1947), ylim = c(0, 12), axes = FALSE, xlab = "", ylab = "")
ph <- list(
...
# phases 3 and 4 are too short to carry their labels inside AND they abut, so each gets a
# row of its own and its label sits beside its own box, tied to it by a leader line
ypos <- c(10.9, 10.9, 9.0, 7.1, 5.2, 3.3); hh <- 0.8
lab_pos <- c("in", "in", "left", "right", "in", "in")
for (i in seq_along(ph)) { p <- ph[[i]]; x0 <- as.numeric(p[1]); x1 <- as.numeric(p[2]); y <- ypos[i]
  rect(x0, y - hh, x1, y + hh, col = p[4], border = base_blue)
  if (lab_pos[i] == "left") { segments(x0 - 0.35, y, x0, y, col = base_blue, lwd = 1)
    text(x0 - 0.45, y, p[3], cex = 0.57, font = 2, col = base_blue, pos = 2)
  } else if (lab_pos[i] == "right") { segments(x1, y, x1 + 0.35, y, col = base_blue, lwd = 1)
    text(x1 + 0.45, y, p[3], cex = 0.57, font = 2, col = base_blue, pos = 4)
  } else text((x0 + x1) / 2, y, p[3], cex = 0.57, font = 2, col = base_blue) }
axx <- seq(1915, 1945, by = 10)
segments(axx, 0.7, axx, 0.95, col = grey); text(axx, 0.2, axx, cex = 0.66, col = grey)
# datable events: dot on the axis, label above it on a clear band, joined by a leader line
ev <- list(c(1925, "gold $4.86"), c(1929, "crash"), c(1931, "UK off gold"), c(1939, "war"))
evy <- c(2.05, 1.45, 2.05, 2.05)
for (i in seq_along(ev)) { e <- ev[[i]]; x <- as.numeric(e[1]); y <- evy[i]
  points(x, 1.05, pch = 19, cex = 0.6, col = accent)
  segments(x, 1.13, x, y - 0.24, col = accent, lwd = 0.8)
  text(x, y, e[2], cex = 0.55, font = 2, col = accent) }
...
text(1931.5, 34, "back toward\n1870s levels", pos = 4, cex = 0.62, col = accent, font = 2)
...
# ---------------------------------------------------------------------------
# 5. The Asian turn: Japan's share of India's cotton-yarn imports
...
text(b[1], 30, "intra-Asian substitution:\nas Europe withdrew,\nOsaka displaced Lancashire", col = base_blue, font = 3, cex = 0.72)
...
text(mean(b) + 0.22, 66, "the weight leaves its old European seat -- for New York, not Asia", col = accent, font = 2, cex = 0.74)
...
text(left - 0.12, 4:1, nm, pos = 2, cex = 0.74, col = base_blue, font = 2, xpd = TRUE)
