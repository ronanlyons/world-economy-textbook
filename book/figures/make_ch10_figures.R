...
text(0.05, 0.80, "NORTH ATLANTIC", col = base_blue, font = 2, cex = 1.25, adj = 0)
text(0.95, 0.80, "ASIA", col = base_blue, font = 2, cex = 1.25, adj = 1)
segments(seq(0.05, 0.95, length.out = 5), 0.38, seq(0.05, 0.95, length.out = 5), 0.62, col = base_blue, lwd = 1)
# the return east: mid-Atlantic (1980) -> toward India-China (2050); west on the left, east on the right
points(0.38, 0.50, pch = 19, cex = 2.6, col = grey); text(0.38, 0.66, "1980", cex = 0.7, col = grey)
points(0.65, 0.50, pch = 19, cex = 3.0, col = accent); points(0.65, 0.50, pch = 21, cex = 3.0, col = base_blue, lwd = 2); text(0.65, 0.66, "~2050", cex = 0.72, col = accent, font = 2)
arrows(0.42, 0.50, 0.605, 0.50, length = 0.08, lwd = 2.2, col = accent)
...
# ---------------------------------------------------------------------------
# 2. The arc in two phases with a turning point between them (stage-setting); output filename unchanged
...
text(8.7, 6.7, "coastal SEZs\n(export\nworkshop)", cex = 0.6, col = accent, font = 2, pos = 4, xpd = TRUE)
text(7.0, 6.9, "Beijing", cex = 0.6, col = base_blue, pos = 4); points(6.8, 6.9, pch = 19, cex = 0.7, col = base_blue)
text(7.45, 4.2, "Shanghai", cex = 0.58, col = accent, pos = 2)
arrows(8.6, 4.6, 9.4, 4.6, length = 0.07, lwd = 2, col = accent); text(9.0, 3.95, "to world\nmarkets", cex = 0.55, col = accent, xpd = TRUE)
...
png("ch10-timeline.png", width = 1750, height = 665, res = 150)
par(mar = c(1.2, 1, 3, 1))
plot(NA, xlim = c(1942, 2018), ylim = c(-2.4, 10), axes = FALSE, xlab = "", ylab = "")
ph <- list(
  c(1944, 1958, "1 reconstruction & the dollar order", "#cdddee"),
  c(1958, 1971, "2 the great reversal at its deepest", "#e0c4c4"),
  c(1971, 1973, "3 the turning point",                  "#f3e6c6"),
...
for (i in seq_along(ph)) { p <- ph[[i]]; x0 <- as.numeric(p[1]); x1 <- as.numeric(p[2]); y <- ypos[i]
  ...
  ty <- if (i == 3) y + 1.25 else y   # narrow box: label sits above it, not across its borders
  text((x0 + x1) / 2, ty, p[3], cex = 0.55, font = 2, col = base_blue) }
axx <- seq(1950, 2010, by = 20)   # decade axis sits at the base, leaving a clear band for the event labels
segments(axx, -1.45, axx, -1.2, col = grey); text(axx, -1.95, axx, cex = 0.66, col = grey)
ev <- list(c(1944, "Bretton Woods"), c(1971, "gold window"), c(1978, "China opens"), c(1991, "India"), c(2010, "China #1 mfr"))
evy <- c(0.30, -0.60, 0.30, -0.60, 0.30)   # staggered rows: neighbouring labels never share a baseline
for (i in seq_along(ev)) { e <- ev[[i]]; ex <- as.numeric(e[1])
  points(ex, 1.05, pch = 19, cex = 0.6, col = accent)
  if (evy[i] < 0) segments(ex, 0.92, ex, evy[i] + 0.28, col = accent, lwd = 0.7)   # leader down to the lower row
  text(ex, evy[i], e[2], cex = 0.58, font = 2, col = accent, xpd = TRUE) }
...
text(82, 32.4, labs2[1], pos = 1, cex = 0.6, col = grey, font = 2)
...
text(mean(b), 33, "China the world's largest manufacturer since 2010 -- the workshop returns east", col = base_blue, font = 3, cex = 0.72)
...
par(mfrow = c(1, 2), mar = c(4.5, 5, 3, 1), oma = c(0, 0, 2.6, 0))
# price
vals <- c(2.90, 11.65)
...
mtext("The Gulf becomes an energy-and-capital pole: the 1973 oil shock", outer = TRUE, line = 0.4, col = base_blue, font = 2, cex = 0.9)
dev.off()
...
text(mean(b), 46.5, "nearly four claims on the metal for every dollar of gold -- the promise was unbackable", col = base_blue, font = 3, cex = 0.68)
